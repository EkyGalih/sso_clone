                    ...
                    <table class="table-auto w-full">
                        ...
                        @foreach($data as $i)
                            <tr class="text-left">
                                ...
                                <td>{{ $i->name }}</td>
                                <td>{{ $i->email }}</td>
                                <td>{{ $i->email_verified_at?->diffForHumans() }}</td>
                                <td>
                                    {{ $i->getRoleNames() }}
                                </td>
                                <td>
                                    <form action="{{ route('user.destroy', $i->id) }}" method="POST" onsubmit="return confirm('{{ __('Are you sure to delete this.') }}')">
                                        @csrf
                                        @method('DELETE')
                                        <div class="flex">
                                            <a href="{{ route('user.show', $i->id) }}" class="text-green-600">
